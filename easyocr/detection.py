...
def get_textboxes(detector, images, canvas_size, mag_ratio, text_threshold, link_threshold, low_text, poly, device, optimal_num_chars=None):
    result = []
    estimate_num_chars = optimal_num_chars is not None

    results = []
    for image in images:
        image_n = image.cpu().numpy()
        image_n = image_n.transpose(1, 2, 0)
        bboxes, polys = test_net(canvas_size, mag_ratio, detector, image_n, text_threshold, link_threshold, low_text, poly, device, estimate_num_chars)

        if estimate_num_chars:
            polys = [p for p, _ in sorted(polys, key=lambda x: abs(optimal_num_chars - x[1]))]

        for i, box in enumerate(polys):
            poly_n = np.array(box).astype(np.int32).reshape((-1))
            result.append(poly_n)
